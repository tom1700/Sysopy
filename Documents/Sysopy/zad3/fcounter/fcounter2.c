...
int search_dir(char * ext,int varg, int warg,char ** argv){
	char * path = argv[1];
	int counter=0;
...
int main(int argc, char **argv){
	...
	if(argc > 2){
		warg = strcmp(argv[2],"-w") == 0 ? 1 : 0;
		varg = strcmp(argv[2],"-v") == 0 ? 1 : 0;
		if(argc > 3){
			if(strcmp(argv[3],"-w")==0) warg = 1;
			if(strcmp(argv[3],"-v")==0) varg = 1;
		}
	}
	exit(search_dir(ext,varg,warg,argv));
}